...
if (test "$?" != "0") then {
	...
	sed -e "s/<script type=\"text\/javascript\">/<script type=\"text\/javascript\" src=\"${a}_archivos\/jquery-2.1.1.min.js\"\/> &/g" /tmp/${a}.html.sinjquery > ${a}.html
} fi;
...
if (test "$?" != "0") then {
	...
	sed -e "s/<script type=\"text\/javascript\">/<script type=\"text\/javascript\" src=\"${a}_archivos\/encuestalocal.js\"\/> &/g" /tmp/${a}.html.sinjs > ${a}.html
} fi;
